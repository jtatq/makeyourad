...
function slotPrompt(slot: RecipeSlot, intake: IntakeForPrompt, mascot: boolean, productId: ProductId): string {
  ...
  const bodies: Record<SlotId, string> = {
    hook: [
      header,
      `ROLE: ${slot.role}`,
      `First line of VO or on-screen text, verbatim: "{{hook_line}}"`,
      `Picture: open on uploaded photo #1 if it shows the home, storefront, or work; otherwise a clean establishing shot in the visual world.`,
      `No logo, no phone number, no end card. Natural sound + VO.`,
      `Hard stop at {{slot_duration}}.`,
    ].join("\n"),
    mascot: [
      header,
      `ROLE: ${slot.role}`,
    ...
    body_1: [
      ...
      `ROLE: ${slot.role}`,
      `BEAT: {{body_1}}`,
      `Build this clip from the customer's photos. If photos are interiors, stay inside. If they are a truck or storefront, lead with that.`,
      `VO continues the hook; do not repeat the hook line.`,
      mascot ? `The mascot may cameo in a corner, but the work is the subject.` : ``,
    ].join("\n"),
    body_2: [
      header,
      `ROLE: ${slot.role}`,
